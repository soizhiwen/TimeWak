...
def calculate_mdd(ori_data, gen_data):
    ...
    mdd = (
        (
            HistoLoss(ori_data[:, 1:, :], n_bins=50, name="marginal_distribution")(
                gen_data[:, 1:, :]
            )
        )
        .detach()
    ...
    def __init__(self, x_real, max_lag=64, stationary=True, **kwargs):
        ...
        if stationary:
            ...
            self.acf_real = non_stationary_acf_torch(
                self.transform(x_real), symmetric=False
            )  # Divide by 2 because it is symmetric matrix

    def compute(self, x_fake):
        if self.stationary:
            acf_fake = acf_torch(self.transform(x_fake), self.max_lag)
        else:
            acf_fake = non_stationary_acf_torch(self.transform(x_fake), symmetric=False)
...
def calculate_acd(ori_data, gen_data):
    ...
    if not torch.is_tensor(gen_data):
        gen_data = torch.tensor(gen_data)
    acf = (
        (ACFLoss(ori_data, name="auto_correlation", stationary=True)(gen_data))
        .detach()
        .cpu()
        .numpy()
    )
