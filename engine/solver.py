...
class Trainer(object):
    ...
    def sample(self, num, size_every, shape=None):
        ...
        while len(samples) < num:
            if self.args.watermark == "TR":
                ...
                gt_patches = np.row_stack([gt_patches, gt_patch])
                ...
                # Filter out NaN samples
                sample = sample[~torch.isnan(sample).any(dim=(1, 2))]
                samples = np.row_stack([samples, sample.detach().cpu().numpy()])
                samples = samples[:num]
            torch.cuda.empty_cache()

        if self.args.watermark == "TR":
            np.save(f"{self.args.save_dir}/gt_patch.npy", gt_patches)
            np.save(f"{self.args.save_dir}/watermarking_mask.npy", watermarking_masks)
    ...
    def detect(self, raw_dataloader, shape=None):
        noises = np.empty([0, shape[0], shape[1]])

        for x in raw_dataloader:
            if self.args.watermark in ["TimeWak", "SpatBDIA"]:
                noise = self.model.inverse_bdia_sample(x)
            else:
                noise = self.model.inverse_fast_sample(x)
            noises = np.row_stack([noises, noise.detach().cpu().numpy()])
